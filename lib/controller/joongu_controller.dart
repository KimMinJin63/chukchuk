...
import 'package:first_project_ver2/service/joongu_service.dart';
import 'package:get/state_manager.dart';

class JoonguController extends GetxController {
  RxList<Restaurant> joonguList = RxList<Restaurant>();
  ...
  getRestaurant() async {
    try {
      print('여기는 중구');
      List<Restaurant> joonguRestaurant = await JoonguService().joonguData();
      joonguList.assignAll(joonguRestaurant);
      print(joonguRestaurant.length);
    } catch (e) {
      print('Error joonguController getRestaurant: $e');
    }
  }
  ...
  collectCategory() async {
    ...
    try {
      var result = await JoonguService().joonguData();
      print('중구 함수 찍힘');
