...
import 'package:first_project_ver2/service/dalseongun_service.dart';
import 'package:get/state_manager.dart';

class DalseongunController extends GetxController {
  RxList<Restaurant> dalseongunList = RxList<Restaurant>();
  ...
  getRestaurant() async {
    try {
      print('여기는 달성군');
      List<Restaurant> dalseongunRestaurant = await DalseongunService().dalseongunData();
      dalseongunList.assignAll(dalseongunRestaurant);
      print(dalseongunRestaurant.length);
    } catch (e) {
      print('Error dalseongunController getRestaurant: $e');
    }
  }
  ...
  collectCategory() async {
    ...
    try {
      var result = await DalseongunService().dalseongunData();
      print('달성군 함수 찍힘');
