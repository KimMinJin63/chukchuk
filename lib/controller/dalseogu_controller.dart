...
import 'package:first_project_ver2/service/dalseogu_service.dart';
import 'package:get/state_manager.dart';

class DalseoguController extends GetxController {
  RxList<Restaurant> dalseoguList = RxList<Restaurant>();
  ...
  getRestaurant() async {
    try {
      print('여기는 달서구');
      List<Restaurant> dalseoguRestaurant = await DalseoguService().dalseoguData();
      dalseoguList.assignAll(dalseoguRestaurant);
      print(dalseoguRestaurant.length);
    } catch (e) {
      print('Error dalseoguController getRestaurant: $e');
    }
  }
  ...
  collectCategory() async {
    ...
    try {
      var result = await DalseoguService().dalseoguData();
      print('달서구 함수 찍힘');
